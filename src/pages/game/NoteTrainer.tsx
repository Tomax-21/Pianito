...
export default function NoteTrainer() {
  ...
  const [liste_note, set_liste_note] = useState<Array<Array<string>>>(() =>
    generateNewList(),
  ); //fct fleché permet d'executer que au lancement
  ...
  function generateNewList() {
    ...
    new_notes = prepareNotesForTrainer(
      get_multiple_random_note({
        nb_note: nb_note,
  ...
  const handlePlayedNote = (noteName: string) => {
    ...
    console.log(
      "note joué, random note",
      noteName,
      liste_note[currentIndex][0],
    );
    if (
      areEnharmonic(
        capitalize(noteName),
        capitalize(liste_note[currentIndex][0]),
      )
    ) {
      ...
      <div className="top-box">
        ...
        <div className="partition-box">
          ...
          <div className="partitions">
            <Partition notes_list={liste_note} show_all_staves={true} />
          </div>
        </div>
      </div>
